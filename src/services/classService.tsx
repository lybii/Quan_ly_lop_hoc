...
export const classService = {
    ...
    getLearningProgress: async (studentId: number) => {
        try {
            const response = await api.get(`/api/classes/learning-process?studentId=${studentId}`);
            return response.data;
        } catch (error) {
            console.error(`Error fetching learning progress for student ${studentId}:`, error);
            return { 
                success: false, 
